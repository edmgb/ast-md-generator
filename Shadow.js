 ...
 */
export const Shadow = (ChosenHTMLElement = HTMLElement) =>
  class Shadow extends ChosenHTMLElement {
    /**
    ...
    constructor(
      ...
      if (typeof options.namespace === "string")
        this.setAttribute("namespace", options.namespace);
      /** @type {string} */
      this.namespace = this.getAttribute("namespace") || "";
      if (options.namespaceFallback)
        this.setAttribute("namespace-fallback", "");
      /** @type {boolean} */
      this.namespaceFallback = this.hasAttribute("namespace-fallback");
      if (typeof options.mobileBreakpoint === "string")
        this.setAttribute("mobile-breakpoint", options.mobileBreakpoint);
      /** @type {string} */
      this.importMetaUrl = (options.importMetaUrl || import.meta.url).replace(
        /(.*\/)(.*)$/,
        "$1"
      );
    }
     ...
     */
    static getMode(mode) {
      return mode === "closed" || mode === "open" || mode === "false"
        ? mode
        : "open";
    }
    ...
    setCss(
      ...
      namespaceFallback = this.namespaceFallback,
      styleNode = this._css,
      appendStyleNode = true,
      maxWidth = this.mobileBreakpoint,
      node = this,
      replaces = []
    ) {
      if (!styleNode) {
      ...
      if (!style) {
        ...
        if (cssSelector !== ":host")
          style = Shadow.cssHostFallback(style, cssSelector);
        if (namespace) {
          if (style.includes("---"))
            console.error("this.css has illegal dash characters at:", this);
        ...
        replaces.forEach(
          (replace) =>
            (style = style.replace(
              new RegExp(replace.pattern, replace.flags),
              replace.replacement
            ))
        );
        ...
        if (Shadow.isMac && style.includes("text-decoration:"))
          style = Shadow.cssTextDecorationShortHandFix(style, node);
    ...
    static cssNamespaceToVarFunc(style, namespace) {
      return style.replace(/var\(--([^,)]*)([^;]*)/g, (match, p1, p2) => {
        if (!p2)
          console.error(
            "this.css cssNamespaceToVarFunc found corrupt css at:",
            this
          );
        return `var(--${namespace}${p1},var(--${p1}${
          p2
            ? this.cssNamespaceToVarFunc(p2, namespace).replace(
                /([^)]*)(.*?)$/,
                "$1)$2"
              )
            : "));"
          ...
          p1.match(/var\(--[^,)]*/g).some(
            (variable) =>
              (p1 = self
                ...
                .getPropertyValue(variable.replace("var(", ""))) !== ""
          );
        return `${p1
          .split(" ")
          .filter((prop) => /\w/g.test(prop))
          .reduce((acc, prop, i) => {
            switch (i) {
              case 0:
    ...
    fetchCSS(fetchCSSParams, hide = true, useController = true) {
      ...
      if (
        ...
        document.body.hasAttribute(
          this.getAttribute("fetch-css") || "fetch-css"
        )
      ) {
        ...
        return new Promise(
          ...
          (resolve) =>
            this.dispatchEvent(
              new CustomEvent(this.getAttribute("fetch-css") || "fetch-css", {
                  ...
                  if (!styleNode) {
                    ...
                    if (
                      this.root.querySelector(
                        this.cssSelector + ` > [_css="${path}"]`
                      )
                    )
                      console.warn(
                        `${path} got imported more than once!!!`,
                        node
                      );
                  }
    ...
    fetchHTML(paths, hide = false, useController = true) {
      ...
      if (
        ...
        useController &&
        document.body.hasAttribute(
          this.getAttribute("fetch-html") || "fetch-html"
        )
      ) {
        ...
        return new Promise(
          ...
          (resolve) =>
            this.dispatchEvent(
              new CustomEvent(this.getAttribute("fetch-html") || "fetch-html", {
    ...
    fetchModules(fetchModulesParams, hide = false, useController = true) {
      if (hide) this.hidden = true;
      if (!Array.isArray(fetchModulesParams))
        fetchModulesParams = [fetchModulesParams];
      if (
        this.isConnected &&
        useController &&
        document.body.hasAttribute(
          this.getAttribute("fetch-modules") || "fetch-modules"
        )
      ) {
        // use: /src/es/components/controllers/fetchHtml/FetchHtml.js instead of fetching here, to use the cache from within the controller
        return new Promise(
              ...
              import(fetchModulesParam.path)
                .then(
                  ...
                  (module) => {
                    ...
                    if (typeof constructorClass === "object")
                      constructorClass =
                        constructorClass[Object.keys(constructorClass)[0]]();
                    if (!customElements.get(fetchModulesParam.name))
                      customElements.define(
                        fetchModulesParam.name,
                        constructorClass
                      );
    ...
    getMobileBreakpoint(
      ...
      return this.hasAttribute("mobile-breakpoint")
        ...
        self.Environment &&
          typeof self.Environment.mobileBreakpoint === "function"
        ? // @ts-ignore ignoring self.Environment error
