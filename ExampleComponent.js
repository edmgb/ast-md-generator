...
export default class Basket extends Shadow() {
  ...
  connectedCallback() {
    ...
    document.body.addEventListener(
      this.getAttribute("active-order-item-event-name") ||
        "active-order-item-event-name",
      this.activeOrderItemEventNameListener
    );
    this.dispatchEvent(
      new CustomEvent(
        this.getAttribute("request-event-name") || "request-event-name",
        {
          bubbles: true,
          cancelable: true,
          composed: true,
  ...
  answerEventNameListener = (
    /** @type {{ detail: { fetch: Promise<any>; }; }} */ event
  ) => {
    event.detail.fetch
      .then(
        (
          /** @type {{ response: { allOrderItemProductTotal: any; }; }} */ productData
        ) => {
          this.count.textContent =
            productData.response?.allOrderItemProductTotal || "0";
        }
      )
  ...
  activeOrderItemEventNameListener = (
    /** @type {{ detail: { fetch: Promise<any>; }; }} */ event
  ) => {
    event.detail.fetch
      .then(
        (
          /** @type {{ response: { allOrderItemProductTotal: any; }; }[]} */ activeItems
        ) => {
          this.count.textContent =
            activeItems[1].response?.allOrderItemProductTotal || 0;
        }
      )
      .catch((/** @type {any} */ error) => {
        this.count.textContent = "0";
  ...
  async renderHTML() {
    return this.fetchModules([
      ...
      (
        /** @type {{ constructorClass: new (arg0: { namespace: any; namespaceFallback: any; mobileBreakpoint: any; }) => any; }[]} */ children
      ) => {
        const icon = new children[0].constructorClass({
  ...
  get count() {
    const element =
      this.root.querySelector("span") || document.createElement("span");
